defmodule PostDeploy.Telemetry do
  @moduledoc false
  ...
  default `PostDeploy.Client`) and `:handler_id` (default
  `:postdeploy_sdk_phoenix_errors`, for `detach/1`). Safe to call more than
  ...
  def attach_phoenix(opts \\ []) do
    handler_id = Keyword.get(opts, :handler_id, :postdeploy_sdk_phoenix_errors)
  ...
  @spec detach(atom()) :: :ok
  def detach(handler_id \\ :postdeploy_sdk_phoenix_errors), do: :telemetry.detach(handler_id)
  ...
  def handle_event([:phoenix, :error_rendered], _measurements, metadata, %{name: name}) do
    if capture?(metadata) do
      ...
      # `PostDeploy.Session`'s severity ordering makes this safe either way.
      PostDeploy.mark_session_crashed()

      PostDeploy.capture_exception(
        normalize(metadata),
