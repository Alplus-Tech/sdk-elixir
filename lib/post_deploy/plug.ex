defmodule PostDeploy.Plug do
  ...
  calls `PostDeploy.set_user/1`:

      plug PostDeploy.Plug

  `{PostDeploy, []}` already attaches the Phoenix error handler. This plug
  ...
  def call(conn, opts) do
    case conn do
      %{assigns: _} ->
        ...
        register_close_session(conn, Keyword.get(opts, :name, PostDeploy.Client))
  ...
  # own parsing and filtering, via `PostDeploy.Scope.set_context/2`.
  ...
  defp attach_request_context(conn) do
    ...
    if map_size(context) > 0, do: PostDeploy.Scope.set_context("request", context)
  rescue
    _ -> :ok
  end
  ...
  # direct qualified call: `postdeploy_sdk` has no compile-time dependency on
  ...
  defp register_close_session(conn, client_name) do
    apply(Plug.Conn, :register_before_send, [
      conn,
      fn conn ->
        PostDeploy.close_session(name: client_name)
        conn
      end
    ])
  rescue
