defmodule PostDeploy.Config do
  @moduledoc false
  ...
  defstruct key: nil,
            ...
            release: nil,
            base_url: "https://ingest.postdeploy.dev",
  ...
  @default_base_url "https://ingest.postdeploy.dev"
  @default_environment "production"

  @doc """
  Builds a `Config` from `start_link/1` opts, falling back in order to
  `Application.get_env(:postdeploy_sdk, :config, [])`, then the matching
  `POSTDEPLOY_*` env var, then the documented default. Raises `ArgumentError` if
  ...
  def new(opts) when is_list(opts) do
    ...
    environment =
      Keyword.get(merged, :environment) || System.get_env("POSTDEPLOY_ENVIRONMENT") ||
        @default_environment
  ...
  `1.0`, i.e. always). Mirrors `sdks/ruby/lib/postdeploy/configuration.rb`'s
