defmodule PostDeploy.Client do
  @moduledoc false

  use GenServer
  require Logger

  alias PostDeploy.{Config, Dedup, Envelope, Id, Transport}

  @type name :: GenServer.name()

  @doc """
  Starts a client. `opts` accepts every `PostDeploy.Config` field plus
  `:name` (default `PostDeploy.Client`, the name every other `PostDeploy`
  ...
  window. Called by `PostDeploy.LoggerHandler` from the logging process; a
  ...
  post-error log window. `PostDeploy.LoggerHandler` uses this so a
  ...
  @doc "Returns the resolved `%PostDeploy.Config{}` (key redacted on inspect). Test/introspection helper."
  ...
  `PostDeploy.Dedup` for why the cache lives here). A cheap, purely
  in-memory `GenServer.call` -- never touches the network -- so it is safe
  to call from `PostDeploy.capture_exception/2`'s own request path. If the
  ...
  # `sdks/ruby/lib/postdeploy/worker.rb`'s bounded queue) -- this guard is its
  ...
  def handle_cast({:enqueue_session, _item}, state)
    ...
    if state.config.debug,
      do:
        Logger.debug(
          "postdeploy_sdk: session queue full (max #{state.config.session_queue_max_items}); dropping session"
        )
  ...
  defp start_async(task_sup, config, fun) do
    ...
    kind, reason ->
      if config.debug,
        do:
          Logger.debug(
            "postdeploy_sdk: failed to start flush task: #{Exception.format(kind, reason, __STACKTRACE__)}"
          )
  ...
  defp await_sync(task_sup, config, fun) do
    ...
    :exit, _ ->
      if config.debug, do: Logger.debug("postdeploy_sdk: flush task did not complete in time")
      :ok
  end
  ...
  defp send_batch(config, items, name) do
    ...
    if byte_size(body) > Config.max_envelope_bytes() do
      if config.debug,
        do: Logger.debug("postdeploy_sdk: dropping oversized envelope (#{length(items)} events)")
    ...
    error ->
      if config.debug,
        do:
          Logger.debug(
            "postdeploy_sdk: internal error while flushing: #{Exception.format(:error, error, __STACKTRACE__)}"
          )
  ...
  defp send_session_batch(config, items, name) do
    ...
    if byte_size(body) > Config.max_envelope_bytes() do
      if config.debug,
        do:
          Logger.debug(
            "postdeploy_sdk: dropping oversized session envelope (#{length(items)} sessions)"
          )
    ...
    error ->
      if config.debug,
        do:
          Logger.debug(
            "postdeploy_sdk: internal error while flushing sessions: #{Exception.format(:error, error, __STACKTRACE__)}"
          )
  ...
  defp deliver_body(%{test?: true}, name, kind, body, _http) do
    PostDeploy.Test.record(name, kind, body)
    :ok
  end
  ...
  defp attach_host_hooks(name, integrations) do
    ...
    hooks =
      if :logger in integrations do
        ...
        :ok = :logger.add_handler(handler_id, PostDeploy.LoggerHandler, %{config: %{name: name}})
    ...
    if :phoenix in integrations do
      handler_id = {:postdeploy_sdk_phoenix, name}
      PostDeploy.Telemetry.attach_phoenix(name: name, handler_id: handler_id)
  ...
  defp detach_host_hooks(hooks) do
    ...
    if hooks.phoenix, do: PostDeploy.Telemetry.detach(hooks.phoenix)
    :ok
  end

  defp logger_handler_id(name), do: :"postdeploy_sdk_logger_#{inspect(name)}"
