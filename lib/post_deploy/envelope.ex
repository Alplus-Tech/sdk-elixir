defmodule PostDeploy.Envelope do
  @moduledoc false

  alias PostDeploy.{Config, Stack}

  @sdk_name "postdeploy-elixir"
  ...
  # and `sdks/ruby/lib/postdeploy/envelope.rb`'s copies of the same -- all three
  ...
  # `sdks/ruby/lib/postdeploy/envelope.rb`'s `MAX_FINGERPRINT_*` copies -- this
  ...
  this layer; `PostDeploy.Config` defaults to 3) attaches
  ...
  `PostDeploy.Session.t()`. Carries no PII: `session.id` is opaque and used
  ...
  @doc """
  Normalizes any `:user` value (per-call opt or `PostDeploy.Scope` ambient
