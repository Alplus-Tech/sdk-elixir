defmodule PostDeploy.Scope do
  @moduledoc false

  @max_breadcrumbs 100
  @pdict_key :__postdeploy_sdk_scope__
  ...
  Normalized through `PostDeploy.Envelope.normalize_user/1` -- both keys are
  ...
  def set_user(user) do
    update(fn scope -> %{scope | user: PostDeploy.Envelope.normalize_user(user)} end)
  end
  ...
  @doc "Returns the current process's ambient scope (an empty `%PostDeploy.Scope{}` if nothing was set)."
  @spec current() :: t()
  def current, do: Process.get(@pdict_key) || %__MODULE__{}

  @doc "Clears the current process's ambient scope. Called by `PostDeploy.Plug` at the start of every request."
  @spec clear() :: :ok
  def clear do
