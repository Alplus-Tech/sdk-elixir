defmodule PostDeploy.LoggerHandler do
  @moduledoc false
  ...
  def log(%{level: level, meta: meta} = log_event, config)
    ...
    case Map.get(meta, :crash_reason) do
      {reason, stacktrace} when is_list(stacktrace) ->
        PostDeploy.capture_exception(normalize_crash_reason(reason),
  ...
  def log(%{level: level} = log_event, config)
    ...
    # client's own `Logger.debug("postdeploy_sdk: ...")` lines must not
    ...
    with false <- Map.get(handler_config, :logger_breadcrumbs) == false,
         ...
         false <- String.starts_with?(message, "postdeploy_sdk") do
      crumb = %{
        category: "log",
        message: message,
      ...
      PostDeploy.Scope.add_breadcrumb(crumb)
  ...
  defp handle_error_log(log_event, config, name) do
    ...
    if is_binary(message) and not String.starts_with?(message, "postdeploy_sdk") do
  ...
  defp capture_plain_report(%{msg: {:string, message}}, name) when is_binary(message) do
    PostDeploy.capture_message(message, "error", mechanism: "logger", name: name)
  end

  defp capture_plain_report(%{msg: {:string, message}}, name) when is_list(message) do
    PostDeploy.capture_message(IO.iodata_to_binary(message), "error",
      mechanism: "logger",
      name: name
    )
  end
