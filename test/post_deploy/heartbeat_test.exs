defmodule PostDeploy.HeartbeatTest do
  @moduledoc """
  `PostDeploy.heartbeat/3` resolves its base URL through a running named
  `PostDeploy.Client`, then `Application.get_env(:postdeploy_sdk, :config, ...)`
  (the same source `PostDeploy.Config` reads), so this file runs
  ...
  setup do
    previous = Application.get_env(:postdeploy_sdk, :config)
    on_exit(fn -> Application.put_env(:postdeploy_sdk, :config, previous || []) end)

    bypass = Bypass.open()
    Application.put_env(:postdeploy_sdk, :config, base_url: "http://localhost:#{bypass.port}")
    {:ok, bypass: bypass}
  end
  ...
  test "defaults to state=finish and hits GET|POST /h/:token with a ping_id", %{bypass: bypass} do
    ...
    assert :ok == PostDeploy.heartbeat("hb_test_token")
  ...
  test "an explicit :start state is sent as ?state=start", %{bypass: bypass} do
    ...
    assert :ok == PostDeploy.heartbeat("hb_test_token", :start)
  ...
  test "an explicit :fail state is sent as ?state=fail", %{bypass: bypass} do
    ...
    assert :ok == PostDeploy.heartbeat("hb_test_token", :fail)
  ...
  test "an unrecognized state falls back to finish instead of raising", %{bypass: bypass} do
    ...
    assert :ok == PostDeploy.heartbeat("hb_test_token", :bogus)
  ...
  test "an unrecognized token (404) is swallowed, not raised", %{bypass: bypass} do
    ...
    assert :ok == PostDeploy.heartbeat("unknown_token")
  end

  test "never raises even against an unreachable endpoint" do
    Application.put_env(:postdeploy_sdk, :config, base_url: "http://127.0.0.1:1")
    assert :ok == PostDeploy.heartbeat("hb_test_token", :finish, no_sleep())
  end

  test "a base_url passed as start_link/1 opts to a running Client wins over app env/POSTDEPLOY_INGEST_URL",
       %{bypass: _bypass} do
    ...
    # `PostDeploy.Client` name, matching `PostDeploy.flush/1`'s own default.
    start_supervised!(
      {Client,
       key: "alp_p_test_key",
