defmodule PostDeploy.LoggerHandlerTest do
  use ExUnit.Case, async: false

  alias PostDeploy.{Client, LoggerHandler}
  ...
  describe "log-line breadcrumbs and the post-error window (issue #47)" do
    test "a sub-error line becomes a log breadcrumb on the logging process's scope" do
      ...
      crumbs =
        PostDeploy.Scope.current().breadcrumbs
    ...
    test "the SDK's own postdeploy_sdk lines and disabled handlers record nothing", %{name: name} do
      PostDeploy.Scope.clear()
      :logger.log(:debug, "postdeploy_sdk: dropping oversized envelope", %{})
      ...
      assert PostDeploy.Scope.current().breadcrumbs
             |> Enum.filter(&(&1[:category] == "log"))
             |> Enum.empty?()
    end
    ...
    test "the window seals and delivers on its own once it elapses", %{bypass: bypass} do
      ...
      PostDeploy.capture_exception(%RuntimeError{message: "boom"}, name: windowed)
    ...
    test "a second capture seals the first so later logs do not leak onto it", %{
      ...
      PostDeploy.Scope.clear()
      first = PostDeploy.capture_exception(%RuntimeError{message: "first"}, name: name)
      second = PostDeploy.capture_exception(%RuntimeError{message: "second"}, name: name)
    ...
    test "a same-signature duplicate keeps the post-error window", %{
      ...
      PostDeploy.Scope.clear()
      first = PostDeploy.capture_exception(%RuntimeError{message: "wrapper"}, name: name)
      _dup = PostDeploy.capture_exception(%RuntimeError{message: "wrapper"}, name: name)
    ...
    test "a duplicate of an earlier exception seals a different pending item", %{
      ...
      PostDeploy.Scope.clear()
      key_id = PostDeploy.capture_exception(%KeyError{key: :a, term: %{}}, name: name)
      wrap_id = PostDeploy.capture_exception(%RuntimeError{message: "wrapper"}, name: name)
      _dup = PostDeploy.capture_exception(%KeyError{key: :a, term: %{}}, name: name)
