defmodule PostDeploy.PlugTest do
  use ExUnit.Case, async: true

  alias PostDeploy.{Client, Scope, Session}
  ...
  test "call/2 clears any leftover ambient scope" do
    ...
    conn = %{assigns: %{}}
    result = PostDeploy.Plug.call(conn, PostDeploy.Plug.init([]))
  ...
  test "call/2 is a no-op passthrough on something without :assigns, never raising" do
    assert :not_a_conn == PostDeploy.Plug.call(:not_a_conn, [])
  end

  test "init/1 returns opts unchanged" do
    assert [foo: :bar] == PostDeploy.Plug.init(foo: :bar)
  end
  ...
  test "call/2 attaches request context from a real conn, with allowlisted headers only" do
    ...
    PostDeploy.Plug.call(conn, PostDeploy.Plug.init([]))
  ...
  test "call/2 attaches no request context for a bare assigns map" do
    PostDeploy.Plug.call(%{assigns: %{}}, PostDeploy.Plug.init([]))
  ...
  describe "session lifecycle over a real %Plug.Conn{} (issue #12)" do
    ...
    test "send_resp closes the session as healthy when nothing was captured", %{
      ...
      conn =
        Plug.Test.conn(:get, "/")
        |> PostDeploy.Plug.call(name: name)
    ...
    test "a handled capture_exception/2 during the request closes the session as errored", %{
      ...
      conn = Plug.Test.conn(:get, "/") |> PostDeploy.Plug.call(name: name)
      PostDeploy.capture_exception(%RuntimeError{message: "handled"}, name: name)
    ...
    test "PostDeploy.mark_session_crashed/0 during the request closes the session as crashed", %{
      bypass: bypass,
      name: name
    } do
      test_pid = self()
      ...
      conn = Plug.Test.conn(:get, "/") |> PostDeploy.Plug.call(name: name)
      PostDeploy.mark_session_crashed()
      PostDeploy.capture_exception(%RuntimeError{message: "unhandled"}, name: name)
