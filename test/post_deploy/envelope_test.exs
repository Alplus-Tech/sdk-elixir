defmodule PostDeploy.CauseCarrier do
  defexception [:message, :cause]
end

defmodule PostDeploy.EnvelopeTest do
  use ExUnit.Case, async: true

  alias PostDeploy.{Envelope, Stack}
  ...
  describe "cause chain" do
    ...
    test "walks a :cause field on the exception struct" do
      ...
      outer = %PostDeploy.CauseCarrier{message: "outer", cause: inner}

      item = Envelope.exception_item("err_field", outer) |> json()

      assert item["exception"]["type"] == "PostDeploy.CauseCarrier"
    ...
    test "bounds the chain at 4 causes" do
      chain =
        Enum.reduce(1..7, %PostDeploy.CauseCarrier{message: "layer 0"}, fn n, acc ->
          %PostDeploy.CauseCarrier{message: "layer #{n}", cause: acc}
        end)
  ...
  describe "source context" do
    ...
    test "attaches pre_context/context_line/post_context to an in_app frame" do
      ...
      stack = [
        {PostDeploy.Stack, :frames, 3, [file: String.to_charlist(@fixture), line: lineno]}
      ]
    ...
    test "skips source context when context_lines is 0" do
      stack = [
        {PostDeploy.Stack, :frames, 3, [file: String.to_charlist(@fixture), line: 10]}
      ]
