defmodule PostDeployTest do
  use ExUnit.Case, async: true

  alias PostDeploy.Client
  ...
  describe "golden envelope" do
    test "a captured exception produces the documented POST /e/errors wire shape", %{
      ...
      # PostDeploy.Client genuinely belongs to the :postdeploy_sdk OTP app (loaded
      ...
      stacktrace = [
        {PostDeploy.Client, :do_flush, 1, [file: ~c"lib/post_deploy/client.ex", line: 42]},
        {:erlang, :apply, 2, []}
      ]

      event_id =
        PostDeploy.capture_exception(exception,
          name: name,
          stacktrace: stacktrace,
          tags: %{"customer" => "acme"},
          in_app_otp_apps: [:postdeploy_sdk]
        )
      ...
      assert %{
               ...
               "exception" => %{
                 ...
                 "stacktrace" => %{
                   "frames" => [
                     %{
                       "function" => "PostDeploy.Client.do_flush/1",
                       "file" => "lib/post_deploy/client.ex",
                       "lineno" => 42,
                       "in_app" => true
                     },
                     frame_2
  ...
  describe "capture_message/3" do
    test "sends a message item with the given level", %{bypass: bypass} do
      ...
      id = PostDeploy.capture_message("disk usage high", "warning", name: name)
    ...
    test "a non-binary message never raises and is stringified", %{bypass: bypass} do
      ...
      id = PostDeploy.capture_message(%{unexpected: :map}, "info", name: name)
  ...
  describe "scope options" do
    test "contexts, breadcrumbs, fingerprint, and user are carried onto the item", %{
      ...
      PostDeploy.capture_exception(%RuntimeError{message: "scoped"},
        name: name,
  ...
  describe "size caps (parity with the JS/Ruby SDKs)" do
    test "oversized contexts are replaced with a truncation marker, not dropped from the send", %{
      ...
      PostDeploy.capture_exception(%RuntimeError{message: "big context"},
        name: name,
        contexts: oversized
      )
    ...
    test "stack frames are trimmed to fit MAX_STACK_TRACE_CHARS, keeping the top frames", %{
      ...
      # (PostDeploy.Client) so `Application.get_application/1` still resolves.
      big_stacktrace =
        for n <- 1..300 do
          {PostDeploy.Client, :"frame_#{n}", 0, [file: ~c"lib/post_deploy/client.ex", line: n]}
        end

      PostDeploy.capture_exception(%RuntimeError{message: "deep stack"},
        name: name,
      ...
      assert hd(frames)["function"] == "PostDeploy.Client.frame_1/0"
    end
    ...
    test "breadcrumb count is capped at SERVER_MAX_BREADCRUMBS, keeping the most recent", %{
      ...
      PostDeploy.capture_exception(%RuntimeError{message: "many crumbs"},
        name: name,
    ...
    test "breadcrumb category/message are truncated per-field", %{bypass: bypass} do
      ...
      PostDeploy.capture_message("check crumb caps", "info",
        name: name,
        breadcrumbs: [
          %{
  ...
  describe "batching" do
    test "flushes automatically once batch_max_items is reached", %{bypass: bypass} do
      ...
      PostDeploy.capture_message("one", "info", name: name)
      PostDeploy.capture_message("two", "info", name: name)
  ...
  describe "fail-safe transport" do
    test "a 500 response is retried up to 3 attempts then swallowed, not raised", %{
      ...
      PostDeploy.capture_message("will 500", "error", name: name)
    ...
    test "a connection failure (endpoint down) is swallowed, not raised", %{bypass: bypass} do
      ...
      PostDeploy.capture_message("endpoint is down", "error", name: name)
      assert :ok == Client.flush(name, 5_000)
    end
    ...
    test "capture_exception/2 never raises even with a garbage stacktrace", %{bypass: bypass} do
      ...
      id =
        PostDeploy.capture_exception(:not_an_exception,
          name: name,
    ...
    test "calling capture_* with no client started never raises and never hits the network" do
      id =
        PostDeploy.capture_exception(%RuntimeError{message: "no client"},
          name: :nonexistent_client
        )

      assert String.starts_with?(id, "err_")
      assert :ok == PostDeploy.flush(name: :nonexistent_client)
    end
  end
  ...
  describe "test/noop mode" do
    test "enabled?: false never enqueues or hits the network", %{bypass: bypass} do
      ...
      id = PostDeploy.capture_exception(%RuntimeError{message: "should be dropped"}, name: name)
      assert String.starts_with?(id, "err_")
      assert :ok == Client.flush(name, 1_000)
    end
  end
  ...
  describe "capture_exception/2 dedup (issue #15)" do
    test "the same error captured twice within the window reports once", %{bypass: bypass} do
      ...
      id1 = PostDeploy.capture_exception(exception, name: name)
      id2 = PostDeploy.capture_exception(exception, name: name)
    ...
    test "a structurally different error is not deduplicated", %{bypass: bypass} do
      ...
      id1 = PostDeploy.capture_exception(%RuntimeError{message: "one"}, name: name)
      id2 = PostDeploy.capture_exception(%RuntimeError{message: "two"}, name: name)
  ...
  describe "scope options (issue #17)" do
    test "PostDeploy.Scope ambient values are carried onto the item and merged with per-call opts",
         %{bypass: bypass} do
      name = start_client(bypass)
      test_pid = self()

      PostDeploy.Scope.clear()
      PostDeploy.Scope.set_user(%{"id" => "ambient_user"})
      PostDeploy.Scope.set_tag("ambient_tag", "yes")
      PostDeploy.Scope.set_context("request", %{"path" => "/orders"})
      PostDeploy.Scope.add_breadcrumb(%{"category" => "nav", "message" => "ambient crumb"})
      ...
      PostDeploy.capture_exception(%RuntimeError{message: "scoped by ambient"},
        name: name,
    ...
    test "an explicit user: nil override clears the ambient user for that one capture", %{
      ...
      PostDeploy.capture_exception(%RuntimeError{message: "cleared user"}, name: name, user: nil)
    ...
    test "the :context convenience option folds into contexts.extra", %{bypass: bypass} do
      ...
      PostDeploy.capture_exception(%RuntimeError{message: "with context"},
        name: name,
        contexts: %{"named" => %{"a" => 1}},
    ...
    test "a user with a non-binary id (a normal Phoenix integer PK) is stringified, not dropped",
      ...
      PostDeploy.capture_exception(%RuntimeError{message: "integer pk user"},
        name: name,
  ...
  describe "sample_rate (issue #17)" do
    test "sample_rate 0.0 drops the capture without hitting the network", %{bypass: bypass} do
      ...
      id = PostDeploy.capture_message("should be sampled out", "info", name: name)
      assert String.starts_with?(id, "err_")
      assert :ok == Client.flush(name, 1_000)
    end
    ...
    test "a sampled-out capture does not poison dedup for a later sampled-in capture of the same error",
      ...
      PostDeploy.capture_exception(exception, name: name)
      PostDeploy.capture_exception(exception, name: name)

      assert {:fresh, "probe_id"} ==
               Client.resolve_dedup(name, PostDeploy.Dedup.signature(exception), "probe_id")
    end
  end
end
